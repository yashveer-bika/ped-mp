...
def main(filepath):
    ...
    for l in f:
        if "Sim Time: " in l:
            if end_of_time:
                ...
                table.append([sim_time, network_level_occupancy, avg_link_tt, avg_delay, stable_def])
                end_of_time = False
    ...
    df = pd.DataFrame(table, columns=["sim time", "network-level occupancy", "avg link tt", "avg delay", "stable-def-region"])
    df.to_csv("oonga.csv")
